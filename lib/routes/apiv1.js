...
function parseResult(key, value) {
  ...
  if (key.indexOf('nd:') !== 0) {
    ...
    result = msgpack.unpack(new Buffer(value, 'binary'));
  return result
}
...
function getKeyDetailsList (key, redisConnection, req, res, next) {
  ...
  redisConnection.lrange(key, startIdx, endIdx, function (err, items) {
    ...
    items = items.map(function (item) {
      return {
        number: i++,
        value: JSON.stringify(parseResult(key, item))
      }
    });
...
function getKeyDetailsHash (key, redisConnection, res, next) {
  redisConnection.hgetall(key, function (err, fieldsAndValues) {
    ...
    for (var field in fieldsAndValues) {
      result[field] = JSON.stringify(parseResult(key, fieldsAndValues[field]));
    }
    var details = {
      key: key,
      type: 'hash',
...
function mapZSetItems (items) {
  ...
  for (var i = 0; i < items.length; i += 2) {
    results.push({
      ...
      value: JSON.stringify(parseResult('nd:', items[i]))
    });
  }
  return results;
}
